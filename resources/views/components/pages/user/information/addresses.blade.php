                ...
                <div class="flex flex-col ">
                    <div class="flex space-x-1 text-base items-center font-medium text-stone-500">
                        <a class="hover:text-stone-800 hover:underline align-middle"
                           href="{{route('user.detail', ['user' => $user])}}">Votre Compte</a>                        <svg class=" font-bold size-4 pt-1" xmlns="http://www.w3.org/2000/svg" fill="none"
                             viewBox="0 0 24 24" stroke-width="3" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5"/>
                        </svg>
                        <div class="">Vos adresses</div>
                    </div>
                    <h1 class=" w-fit text-xl font-semibold">Vos adresses</h1>
                </div>
                <div>
                ...
                @foreach($user->addresses as $address)
                    <div class="w-96 h-80 shadow border px-4 flex py-4 flex-col justify-between border-stone-300 rounded-xl border">
                        <div>
                            ...
                            <div>{{$address->city}}, {{$address->zip_code}}</div>
                            <div>{{$address->country}}</div>
                            <div>Numéro de téléphone: {{$address->user->phone}}</div>
                        </div>

                        <div class="flex text-sm text-cyan-500">
                            <a href="{{route('user.addresses.edit', ['user' => $user,'address'=> $address])}}"
                        ...
                        @if(!$address->is_default)
                                ...
                                <form action="{{route('user.addresses.delete', ['user' => $user, 'address' => $address])}}"
                                      method="POST">
                                    @csrf
                                    <button type="submit"
                                            class="hover:underline px-3">
                                ...
                                <div class="text-black">|</div>
                                <a href="{{route('user.addresses.default', ['user' => $user,'address'=> $address])}}"
                                   class="hover:underline px-3">
                                    Définir par défaut
                                </a>
                            @endif
